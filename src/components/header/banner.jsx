...
function Banner() {
  ...
  return (
    <header
      className={`${
        !active && "hidden"
      } relative bg-gray-10 text-white max-sm:h-[78px] max-laptop:h-[49px] h-[63px] flex items-center justify-between sm:justify-center max-sm:px-4 max-sm:pt-10 max-sm:pb-5`}
    >
      ...
      <div className="font-medium max-sm:text-[12px] max-laptop:text-sm text-lg flex justify-center max-laptop:gap-1.5 gap-2.5 relative">
        ...
        <Link
          href={`/properties`}
          className="underline max-small-mobile:hidden"
        >
          Learn More
        </Link>
      </div>
      <button
        ...
        className="sm:absolute top-1/2 transform sm:-translate-y-1/2 flex justify-center items-center max-laptop:w-6.5 max-laptop:h-6.5 w-8 h-8 bg-white/10 rounded-full right-[29px] cursor-pointer"
      >
        <Image
          width={24}
          height={24}
